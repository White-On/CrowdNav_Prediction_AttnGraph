...
class Human(Agent):
    ...
    def __init__(self, delta_t:float, is_moving=True, is_visible=True, desired_speed=1.0, radius=0.2, FOV=np.pi, **kwargs):
    ...
    def predict_what_to_do(self, *other_agent_state:list) -> list:
        ...
        simulation = rvo2.PyRVOSimulator(self.delta_t, *params, self.radius, self.desired_speed)
        # add self to simulation
        simulation.addAgent((self.coordinates[0], self.coordinates[1]), *params, self.radius + safe_distance, self.desired_speed, (self.speed[0], self.speed[1]))
