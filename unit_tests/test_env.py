...
@pytest.fixture(params=  np.arange(0, 30, 5), ids=[f"amount_of_agents={i}" for i in range(0, 30, 5)])
...
def test_human_generation(random_group_human):
    ...
    assert Human.apply(lambda x: x.coordinates != [None,None]) == [True] * len(random_group_human)
